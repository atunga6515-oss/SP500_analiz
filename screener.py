...
from sp500_symbols import SP500_30, SP500_100, SP500_ALL, SP500_SECTORS

# Geriye uyumlu isimler — S&P 500 verisiyle dolduruldu (downstream kod değişmeden çalışır)
BIST30_SYMBOLS = list(SP500_30)
BIST100_SYMBOLS = list(SP500_100)
BIST_ALL_SYMBOLS = list(SP500_ALL)
...
# GICS sektör haritası (SP500_SECTORS'tan türetilir)
SECTOR_MAP = {}
for _sym, _sec in SP500_SECTORS.items():
    SECTOR_MAP.setdefault(_sec, []).append(_sym)

def get_sector(symbol: str) -> str:
    """Hissenin GICS sektörünü döndürür."""
    return SP500_SECTORS.get(symbol.upper().replace(".", "-"), "Other")
...
def run_screener(symbol_list: list, username: str, progress_bar=None, max_workers: int = 5) -> pd.DataFrame:
    ...
    # 1. Piyasa Rejimi Onayı (S&P 500 Endeksi Analizi)
    xu100_df = fetch_data("^GSPC", interval="1d", period="1y")
