...
def get_layered_data(ticker: str, current_user: str = Depends(get_current_user)):
    try:
        yf_ticker = ticker.upper().strip().replace(".IS", "").replace(".", "-")
...
@router.get("/{ticker}")
def fetch_comprehensive_analysis(ticker: str, lang: str = None, current_user: str = Depends(get_current_user)):
    try:
        from core.analysis_service import run_deep_analysis

        final_payload = run_deep_analysis(ticker, lang=lang)
