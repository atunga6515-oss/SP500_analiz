...
def get_istanbul_now():
    """Piyasa yerel saatini (ABD/Doğu) döndürür. (İsim geriye uyumluluk için korundu.)"""
    return datetime.now(TR_TZ)

# Açık isimli alias (yeni kod bunu kullanmalı)
def get_market_now():
    return datetime.now(TR_TZ)
...
def _make_ticker(symbol: str) -> str:
    """
    Sembolü yfinance formatına normalize eder (S&P 500 / ABD).
    - Borsa kodu eki EKLEMEZ (ABD sembolleri ek istemez).
    - Eski .IS ekini temizler (geriye uyumluluk).
    - Endeks/özel semboller (^, =) olduğu gibi bırakılır.
    - Sınıf hisseleri için nokta -> tire (BRK.B -> BRK-B).
    """
    ...
    if sym.startswith('^') or '=' in sym:
        return sym
    # yfinance sınıf hisselerinde tire kullanır
    sym = sym.replace('.', '-')
    return sym
