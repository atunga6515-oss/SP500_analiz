...
export default function IndicatorsDashboard() {
    ...
    useEffect(() => {
        ...
        api.get(`/analysis/layered-data?ticker=${selectedTicker}`)
            ...
            .catch((err) => {
                console.error(err);
                if (isMounted) toast.error(t("ind.dataError").replace("{t}", selectedTicker || ""));
            })
            .finally(() => {
        ...
        <div className="flex w-full h-[calc(100vh-64px)] bg-gray-900 text-white overflow-hidden">
            ...
            <div className="w-64 border-r border-gray-800 bg-gray-900/50 flex flex-col">
                <div className="p-4 border-b border-gray-800 flex items-center justify-between">
                    <h2 className="text-sm font-semibold tracking-wider text-gray-400 uppercase">{t("ind.watchlist")}</h2>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-1">
                    {watchlist.length === 0 ? (
                        <div className="text-center p-4 text-gray-500 text-sm mt-10">
                            {t("ind.listEmpty")}<br/><br/>{t("ind.listEmptyHint")}
                        </div>
                    ) : (
                        ...
                        <div className="ml-auto flex items-center gap-3 relative">
                            ...
                            {showHistoryDropdown && (
                                <div className="absolute top-10 right-32 w-64 bg-gray-800 border border-gray-700 rounded shadow-xl z-50">
                                    <div className="p-2 text-xs text-gray-400 border-b border-gray-700 font-bold uppercase tracking-wider">{t("ind.recentAnalyses")}</div>
                                    <div className="max-h-60 overflow-y-auto">
                                        {aiHistory.length === 0 ? (
                                            <div className="p-3 text-sm text-gray-500 italic">{t("ind.noRecord")}</div>
                                        ) : (
            ...
            {showPreModal && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
                    <div className="bg-[#1e2329] border border-gray-700 rounded-xl shadow-2xl p-6 w-[450px] flex flex-col gap-4">
                        <h3 className="text-lg font-bold text-yellow-500 flex items-center gap-2">{t("ind.aiStarting")}</h3>
                        ...
                        <div className="flex gap-3 justify-end mt-2">
                            <button onClick={() => setShowPreModal(false)} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded text-sm font-semibold transition-colors">{t("ind.cancel")}</button>
                            <button onClick={handleAIAnalysis} disabled={aiLoading} className="px-4 py-2 bg-purple-700 hover:bg-purple-600 text-white rounded text-sm font-semibold transition-colors flex items-center gap-2">
                                {aiLoading ? <span className="animate-spin">⏳</span> : "✨"} {t("ind.startAnalysis")}
                            </button>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="bg-[#1e2329] border border-gray-700 rounded-xl shadow-2xl p-8 w-[650px] flex flex-col gap-5">
                        <div className="flex justify-between items-center border-b border-gray-800 pb-4">
                            <h3 className="text-base font-bold text-yellow-500 uppercase tracking-wider flex items-center gap-2">
                                🤖 {aiResult.ticker || selectedTicker} - {t("ind.decisionMatrix")}
                            </h3>
                        ...
                        <div className="flex gap-3 justify-end mt-4 border-t border-gray-800 pt-5">
                            <button onClick={() => setShowResultModal(false)} className="px-5 py-2.5 bg-gray-700 hover:bg-gray-600 rounded text-sm font-semibold transition-colors">{t("ind.close")}</button>
                            <button onClick={handleSaveAnalysis} className="px-5 py-2.5 bg-blue-700 hover:bg-blue-600 text-white rounded text-sm font-semibold transition-colors">{t("ind.saveResult")}</button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {aiLoading && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
                    <div className="bg-[#1e2329] border border-gray-700 rounded-xl shadow-2xl p-8 flex flex-col items-center gap-5">
                        ...
                        <p className="text-lg text-gray-300 font-medium">{t("ind.aiReading")}</p>
                    </div>
                </div>
            )}
        </div>
