...
export default function HeatmapPage() {
    ...
    const fetchHeatmap = async () => {
        try {
            ...
            setError(t("heatmap.loadError") + err.message);
        } finally {
            setLoading(false);
        }
    };
    ...
    return (
        <div className="flex w-full h-full p-6 flex-col bg-[var(--color-b-bg)] text-[var(--color-b-text)] overflow-auto">
            <div className="mb-6">
                <h1 className="text-3xl font-bold text-white mb-2">{t("heatmap.title")}</h1>
                <p className="text-[var(--color-b-muted)]">{t("heatmap.subtitle")}</p>
            </div>
