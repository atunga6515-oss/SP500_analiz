...
export default function AIAnalyzeModal({ isOpen, onClose, ticker, price, rsi, macd_signal, trend, note }: AIModalProps) {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            let errDetail = error.response?.data?.detail || t("aim.genericError");
            ...
            <div className="bg-[#181a20] border border-gray-800 rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
                <div className="flex items-center justify-between p-4 border-b border-gray-800 bg-[#1e2329]">
                    <h3 className="font-bold text-white flex items-center gap-2">
                        <span>🤖</span> {t("aim.title")} <span className="text-[var(--color-b-yellow)]">{ticker}</span>
                    </h3>
                ...
                <div className="p-6 overflow-y-auto">
                    {step === "CONFIRM" && (
                        <div className="text-center py-6">
                            ...
                            <h4 className="text-xl font-bold text-white mb-2">{t("aim.confirmTitle").replace("{t}", ticker)}</h4>
                            <p className="text-gray-400 mb-6">{t("aim.confirmDesc")}</p>

                            <div className="bg-[#1e2329] p-4 rounded-lg inline-block mb-8 border border-gray-800 min-w-[200px]">
                                <p className="text-sm text-gray-400 mb-1">{t("aim.remainingQuota")}</p>
                                {quota === null ? (
                                    <span className="text-xl text-white animate-pulse">{t("aim.loading")}</span>
                                ) : (
                    ...
                    {step === "LOADING" && (
                        <div className="flex flex-col items-center justify-center py-12">
                            ...
                            <h3 className="text-xl font-bold text-white mb-2 animate-pulse">{t("aim.preparing")}</h3>
                            <p className="text-gray-400 text-sm">{t("aim.analyzing")}</p>
                        </div>
                    )}
                    ...
                    {step === "ERROR" && (
                        <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-6 text-center">
                            ...
                            <button onClick={onClose} className="mt-4 px-4 py-2 border border-red-500/50 text-red-400 rounded hover:bg-red-500/10">{t("aim.close")}</button>
                        </div>
                    )}
                </div>
            </div>
