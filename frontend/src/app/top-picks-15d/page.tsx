    ...
    return (
        ...
        <div className="flex w-full h-full p-6 flex-col bg-[var(--color-b-bg)] text-[var(--color-b-text)] overflow-y-auto">
            <div className="flex justify-between items-end mb-6">
                <div>
                    <h1 className="text-3xl font-bold text-white mb-2">{t("tp15.title")}</h1>
                        ...
                        <table className="w-full text-left text-sm border-collapse">
                            <thead className="bg-[#1e2329] text-[var(--color-b-muted)] sticky top-0">
                                <tr>
                                    ...
                                    <th className="p-4 border-b border-[var(--color-b-border)] font-semibold text-center tip" data-tip={t("tip.signals")}>{t("tp.colSignals")}</th>
                            ...
                            <tbody>
                                {sortedPicks.map((row: any, i: number) => {
                                    ...
                                    <tr key={i} className="hover:bg-[#1e2329] transition-colors border-b border-[var(--color-b-border)]">
                                        ...
                                        <td className="p-4 text-center tip" data-tip={_sigLines.map((l: string) => tv(l)).join("\n")}>
                                            {_sigBadges.length
            ...
            {chartModalOpen && (
                <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setChartModalOpen(false)}>
                    <div className="bg-[#181a20] border border-[var(--color-b-border)] rounded-lg shadow-xl w-[90vw] max-w-5xl flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
                        <div className="bg-[#1e2329] p-4 flex justify-between items-center border-b border-[var(--color-b-border)]">
                            <h3 className="font-bold text-lg text-white">📈 {chartTicker} - {t("tp15.chartTitle")}</h3>
